var zBase = {
  ...
  init:function(){
    if (screen.width>=600){
      ...
      $(".adpics img").css("width",600);
      $(".adpics img").css("height",200);
    }
    else{
      $(".slide").css("height",screen.width/3);
      $(".adpics img").css("width",screen.width);
      $(".adpics img").css("height",screen.width/3);
    }
    this.slide = this.$id('slide');
    this.img_div = this.$c('adpics')[0],
    this.slide_btn = this.$tag('a',this.$c('slidebtn')[0]);
  ...
  play:function(){
    this.slide.timer = setInterval(function(){
      ...
      if(screen.width>=600){
        zBase.animate(zBase.img_div,zBase.config.direct,-zBase.config.index*600);
      }
      else{
        zBase.animate(zBase.img_div,zBase.config.direct,-zBase.config.index*screen.width);
      }
